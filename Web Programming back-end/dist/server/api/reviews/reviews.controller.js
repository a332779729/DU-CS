...
var _reviews = require('./reviews.model');

var _users = require('../users/users.model');

var _recipes = require('../recipes/recipes.model');
...
function index(req, res) {
  _reviews.Review.find().populate('userCreatingReview').exec().then(function (reviews) {
    res.json({
      reviews
    });
  }).catch(function (err) {
...
function show(req, res) {
  _reviews.Review.findById(req.params.id).populate('userCreatingReview').exec().then(function (existingReview) {
    /*
...
async function create(req, res) {
  ...
  try {
    console.log(userId);
    user = await _users.User.findById(userId);
    console.log(user);
    if (!user) {
      res.status(400);
  ...
  try {
    ...
    recipe = await _recipes.Recipe.findById(recipeId);
    console.log(recipe);
    if (!recipe) {
      res.status(400);
  ...
  try {
    let createdReview = await _reviews.Review.create(review);
...
function update(req, res) {
  ...
  _reviews.Review.findById(req.params.id).exec()
  // Update review
  .then(function (existingReview) {
    ...
    if (existingReview) {
      ...
      */
      return _promise2.default.all([existingReview.increment().save()]);
    } else {
      // Review was not found
...
function destroy(req, res) {
  _reviews.Review.findById(req.params.id).exec().then(function (existingReview) {
    if (existingReview) {
      /*
        ...
        are deleted
       */
      return _promise2.default.all([existingReview.remove()]);
    } else {
      return existingReview;
    }
  })
