...
function show(req, res) {
  _users.User.findById(req.params.id).populate('address').exec().then(function (existingUser) {
    /*
...
function create(req, res) {
  ...
  */
  .then(function (createdAddress) {
    ...
    */
    return _users.User.create(user);
  })
...
function update(req, res) {
  ...
  _users.User.findById(req.params.id).populate('address').exec()
  ...
  .then(function (existingUser) {
    ...
    if (existingUser) {
      ...
      */
      return _promise2.default.all([existingUser.address.increment().save(), existingUser.increment().save()]);
    } else {
      // User was not found
      return existingUser;
    }
  ...
  _users.User.findById(req.params.id).populate('address').exec().then(function (existingUser) {
    if (existingUser) {
      /*
        ...
        are deleted
       */
      return _promise2.default.all([existingUser.address.remove(), existingUser.remove()]);
    } else {
      return existingUser;
    }
  })
