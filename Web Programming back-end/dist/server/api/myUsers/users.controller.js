...
function show(req, res) {
  ...
  var findUser = _users2.default.findById(id);
...
function create(req, res) {
  ...
  if (typeof name === "string" && typeof address === "string" && typeof age === "number" && age > 0) {
    ...
    res.status(201);
    res.json(_users2.default.create(user));
  } else {
    res.status(400);
...
function upsert(req, res) {
  ...
  var result = _users2.default.findOneAndUpdate(myUser);
  if (result == true) {
    res.status(200);
    res.json(myUser);
  } else {
...
function destroy(req, res) {
  ...
  var result = _users2.default.remove(myUser);
  if (result == false) {
    res.status(404);
    res.json({ message: "Not Found" });
  } else {
