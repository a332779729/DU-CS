...
function show(req, res) {
  _users.User.findById(req.params.id).exec().then(function (existingUser) {
    /*
...
async function create(req, res) {
  ...
  */
  ...
  try {
    let createdUser = await _users.User.create(user);
...
function update(req, res) {
  ...
  _users.User.findById(req.params.id).exec()
  ...
  .then(function (existingUser) {
    ...
    if (existingUser) {
      ...
      */
      return _promise2.default.all([existingUser.increment().save()]);
    } else {
      // User was not found
      return existingUser;
    }
  ...
  _users.User.findById(req.params.id).exec().then(function (existingUser) {
    if (existingUser) {
      /*
        ...
        are deleted
       */
      return _promise2.default.all([existingUser.remove()]);
    } else {
      return existingUser;
    }
  })
